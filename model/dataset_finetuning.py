...
class MidiDataset(Dataset):
    ...
    This version is for fine-tuning:
    - It uses a metadata CSV for tempo normalization.
    - It merges all tracks from each MIDI file into a single piano roll.
    ...
    def __init__(self, midi_dir: str, metadata_csv_path: str, num_bars: int, steps_per_bar: int = 16, 
        ...
        try:
            ...
            print(f"Warning: Metadata CSV file not found at {metadata_csv_path}. Will rely on default BPM.")
    ...
    def _process_full_midi_file(self, file_path: str, file_name: str) -> Optional[Tuple[np.ndarray, float]]:
        """
        Processes a MIDI file by normalizing tempo and merging all tracks into a single piano roll.
        """
        ...
        # --- ZMIANA: Przetwarzanie wszystkich ścieżek w pliku ---
        # Ta logika jest idealna dla plików, które mają już tylko jedną, właściwą ścieżkę.
...
def prepare_dataloaders(split_ratios=(0.85, 0.15), seed: int = 42):
    ...
    if train_size == 0 or val_size == 0:
        raise ValueError(f"Dataset too small to split. Total samples: {total_size}")
    ...
    print("Finished preparing dataset for fine-tuning.\n")
